        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            >
              ...
              <div>
                <p className="text-[18px] font-bold text-text-main mb-2">消息已发送！</p>
                <p className="text-[13px] text-text-muted leading-relaxed">
                  感谢你的反馈。我们通常在工作日 24 小时内回复，请留意你的联系方式。
                </p>
              </div>
              <div className="bg-bg-card rounded-[16px] p-4 w-full border border-border/50 space-y-2">
                <p className="text-[12px] text-text-muted text-left">
                  <span className="font-medium text-text-main">主题：</span>{topic}
                </p>
                <p className="text-[12px] text-text-muted text-left line-clamp-2">
                  <span className="font-medium text-text-main">内容：</span>{message}
                </p>
              </div>
            </motion.div>
          ) : (
              ...
              <div className="space-y-2">
                {CONTACT_INFOS.map((info, i) => (
                  <motion.div
                    ...
                    className="bg-bg-card rounded-[16px] flex items-center justify-between px-4 py-3.5 border border-border/50 shadow-[0_2px_8px_rgba(41,37,38,0.04)]"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-bg-page rounded-[10px] flex items-center justify-center shrink-0">
                        <info.icon size={16} strokeWidth={2} className="text-primary" />
                      </div>
                      <div>
                        <p className="text-[11px] text-text-muted">{info.label}</p>
                        <p className="text-[13px] font-medium text-text-main">{info.value}</p>
                      </div>
                    </div>
                    {info.label !== '响应时间' && (
                      <motion.button
                        ...
                        className="w-8 h-8 bg-bg-page rounded-[9px] flex items-center justify-center"
                      >
                        {copied === info.value ? (
                          <Check size={14} strokeWidth={2.5} className="text-green-600" />
                        ) : (
                          <Copy size={14} strokeWidth={2} className="text-text-muted" />
                        )}
                      </motion.button>
                    )}
                  </motion.div>
                  ...
                  {TOPICS.map((t) => (
                    <motion.button
                      ...
                      className={`px-4 py-2 rounded-[100px] text-[12px] font-medium border transition-all ${
                        topic === t
                          ? 'bg-primary text-[#FDFDFD] border-primary'
                          : 'bg-bg-card text-text-sub border-border'
                      }`}
                    >
                      {t}
                    </motion.button>
